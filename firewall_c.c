...
print_usage(void)
{
	printf("Usage: Firewall_C RULE_OPTIONS..\n"
	       "firewall_c implements an exact match algorithm, where "
...
/*
 * The function sends a command to a firewall_c module via a device file.
 */
static void
send_instruction(struct firewall_c_ctl *ctl)
{
	FILE *fp;
	int byte_count;
...
view_rules(void)
{
	...
	printf("I/O  "
	       "Src_Addr           Src_Mask           Src_Port "
	       "Des_Addr           Des_Mask           Des_Port Proto\n");
	while((byte_count = fread(buffer, 1, sizeof(struct firewall_c_rule), fp)) > 0) {
		rule = (struct firewall_c_rule *)buffer;
 ...
 */
static int
parse_arguments(int argc, char **argv, struct firewall_c_ctl *ret_ctl)
{
	int opt;
	int64_t lnum;
	int opt_index;
	struct firewall_c_ctl ctl = {};
	struct in_addr addr;
	...
	while(1) {
		...
		case 'c':	/* Protocol number */
			...
			break;
		case 'a':	/* Add rule */
			if(ctl.mode != firewall_c_NONE) {
				printf("Only one mode can be selected.\n");
				return -1;
			}
			ctl.mode = firewall_c_ADD;
			break;
		case 'r':	/* Remove rule */
			if(ctl.mode != firewall_c_NONE) {
				printf("Only one mode can be selected.\n");
	...
	if(ctl.mode != firewall_c_VIEW && ctl.rule.in == -1) {
